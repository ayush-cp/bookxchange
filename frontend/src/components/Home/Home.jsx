      ...
      >
        ...
        ref={(e)=>(imageRef.current[0]=e)}
          className={`transition-all ease-linear duration-500 z-0 md:w-[50%] w-[60%] md:h-[75%] h-[80%] absolute bottom-[5%] ${
            visibleItem[0] ? "right-[-5%] opacity-100 blur-0" : "right-[-10%] opacity-0 blur-lg"
          }`}
        >
          <img
            src={hero}
        ...
        <div className="z-10 md:w-1/2 w-full h-max flex flex-col items-center gap-4">
          <h1 data-index={1} ref={(e)=>(imageRef.current[1]=e)} className={`${visibleItem[1]?'scale-100 opacity-100 blur-0':'scale-60 opacity-0 blur-lg'} flex transition-all ease-linear duration-500 flex-col gap-4 md:text-5xl sm:text-3xl text-2xl font-semibold text-gray-800`}>
            <span>Share Knowledge</span>{" "}
            <span className="text-emerald-700 ml-20">Increase Knowledge</span>
          </h1>
          <div data-index={2} ref={(e)=>(imageRef.current[2] = e)} className={`flex flex-row transition-all ease-out duration-500 gap-4 ${visibleItem[2]?'opacity-100 blur-0':'opacity-0 blur-lg'}`}>
          ...
          <h3 data-index={3} ref={(e)=>(imageRef.current[3]=e)} className={`md:text-xl sm:text-lg text-md font-semibold transition-all ease-in duration-500 text-black ${visibleItem[3]?'blur-0':'blur-lg'}`}>
